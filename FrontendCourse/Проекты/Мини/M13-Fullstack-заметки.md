...
Приложение «Заметки»: пользователь логинится, создаёт, редактирует и удаляет свои заметки. Может сделать заметку публичной, тогда она появляется в общей ленте на главной. Всё на Server Components и Server Actions, без API-роутов (кроме auth.js callback).
...
3. **`/me`:** личный кабинет, список своих заметок (публичные и приватные). Защищено session-check.
4. **`/notes/new`:** форма создания. Server Action создаёт запись.
5. **`/notes/[id]`:** публичная просмотрщица заметки. `generateMetadata` с title и description.
...
- Supabase (Postgres) и Drizzle ORM.
...
1. `npx create-next-app@latest notes-app` (App Router, TS, ESLint, Tailwind или CSS на выбор).
2. Подключи Supabase: создай проект, скопируй URL и anon key в `.env.local`.
3. Настрой Drizzle: `drizzle-orm`, `drizzle-kit`, schema плюс migrations.
4. Схема `notes` плюс генерация миграций: `npx drizzle-kit push`.
...
8. `/me`: auth-check плюс список своих заметок.
9. Формы create / edit: Server Actions плюс Zod validation.
...
## Что дальше
Дальше идёт модуль 14 «Деплой и финал»: расширение до P3 (комментарии, теги, поиск), оптимизация Vercel (Image, Font, edge), финальные метрики Lighthouse, готовность к резюме.
