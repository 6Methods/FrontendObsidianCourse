...
Переписать все файлы проекта M10 из `.jsx` в `.tsx` (а `.js` в `.ts`), добавить типы для props, state, API, форм. Без `any`. После рефакторинга проект должен работать точно так же, как раньше.

## Требования

### Функциональность
- Весь функционал M10 работает: роуты, каталог, корзина, checkout, персист.
- Деплой на Vercel (можно старый проект переключить на новую ветку или новый деплой).

### TypeScript-правила
1. **`strict: true` в tsconfig:** `noImplicitAny`, `strictNullChecks` и всё остальное из `strict`.
2. **Ноль `any`.** Если реально не можешь, бери `unknown` плюс narrowing. Комментарий с причиной, если оставляешь.
3. **Props типизированы** для каждого компонента через `type Props = { ... }` или `interface`.
4. **Типизирован Context.** `CartContext` через generic или явный тип, хук `useCart` выбрасывает при отсутствии Provider.
...
7. **Форма checkout** — zod-схема плюс RHF с `zodResolver`. Тип формы через `z.infer<typeof CheckoutSchema>`.
...
- Обнови Vite-проект: `vite.config.ts`, переименуй `main.jsx` в `main.tsx`, `App.jsx` в `App.tsx`.
...
- Обнови README: секция «TypeScript: что поменялось».
...
1. В копии M10 (`git checkout -b ts-refactor`) поставь `npm install -D typescript @types/node`.
2. Создай `tsconfig.json` (на базе Vite react-ts шаблона).
3. Переименуй `main.jsx` в `main.tsx`, `App.jsx` в `App.tsx`.
4. Компоненты из `pages/` и `components/` переводи по одному. Типизируй props, state, events.
...
7. Форма checkout: вынеси `CheckoutSchema` плюс `type CheckoutForm = z.infer<...>`.
...
- [ ] Context типизирован, хук защищён от отсутствия Provider.
- [ ] Zod-схема для API-ответа (минимум Product).
- [ ] Форма checkout: `zodResolver` плюс `z.infer` для типа формы.
- [ ] `useState<T>` с явным типом там, где TS не выводит.
- [ ] Функциональность M10 не сломана.
- [ ] Прод работает.

## Что дальше
Дальше — [[M11b-Pokedex-на-TS|M11b Pokédex на TS]] с нуля. Там TS проектируется сразу, а не ретроспективно.
