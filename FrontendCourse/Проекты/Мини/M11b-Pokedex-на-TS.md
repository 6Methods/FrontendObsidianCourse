...
3. **Поиск** по имени, debounce 300 мс.
...
- UI-библиотек (MUI, Chakra). Свои компоненты плюс CSS.

## Этапы

1. `npm create vite@latest pokedex-ts -- --template react-ts`, поставь `react-router-dom`, `@tanstack/react-query`, `@tanstack/react-query-devtools`, `zod`.
...
7. `context/FavoritesContext.tsx`: типизированный провайдер плюс хук `useFavorites`.
...
- [ ] Поиск и фильтр через `useSearchParams`.
- [ ] Избранное persist в localStorage.
- [ ] Ноль `any` без обоснования.
- [ ] Прод работает, 151 покемон есть (или работает пагинация).

## Что дальше
Дальше идёт модуль 12 «Build tools и тестирование»: Vite в деталях, ESLint, Prettier, Vitest, Testing Library, Playwright. Добавим зелёную планку «тесты проходят» к твоим проектам.
