...
Мини-проект после модуля 08. Две цели в одной задаче: отработать feature-branch → PR → merge с конфликтом и отладить 3–4 подсунутых бага через DevTools. Практика максимально приближена к реальной работе.
...
2. Прогони код через 4 подсунутых бага (список ниже), найди и почини каждый через DevTools, без `console.log`-ов в коде.
...
5. **Merge-конфликт.** Синхронизируй feature с main (`git pull origin main` в feature). Если конфликта не возникает — специально создай: отредактируй README и в feature-ветке (та же строка, что правил в main). Разреши конфликт, запушь.
...
В своём проекте внеси 4 искусственных бага в отдельной ветке `bugs/debug-session` (не мерджи её). Потом найди и опиши каждый. Цель не починить, а доказать, что ты нашёл и понял проблему через DevTools.
...
## Этапы
1. Выбери базовый проект (M06 или M07), клонируй или открой.
