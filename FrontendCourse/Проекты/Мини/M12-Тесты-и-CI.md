...
Мини-проект после модуля 12. Берёшь свой M11a (SPA-магазин на TS) — обкладываешь тестами: юнит (Vitest), компонентные (Testing Library), E2E (Playwright). Поднимаешь CI на GitHub Actions — без зелёной галочки PR не мёржится. Это то, что делает код «боевым», а не «домашним».
...
### Тесты
1. **Vitest (unit)** — минимум 3 теста на утилиты/хуки. Кандидаты: `useDebounce`, `useCart` (редьюсер корзины), форматтер цены.
...
4. **Покрытие** (unit+component) — `--coverage` в Vitest, минимум 50% по изменённым файлам.

### Lint и форматирование
- ESLint 9 flat config, `eslint-plugin-react-hooks`, `typescript-eslint`, `eslint-plugin-jsx-a11y`.
- Prettier: `.prettierrc` + `eslint-config-prettier` (отключить конфликтующие правила).
...
- Playwright-job (опционально отдельно): `npx playwright install --with-deps chromium` + прогон E2E.
...
- Тестов на стили/классы (`toHaveClass("btn-primary")`) — тестируй поведение, не верстку.
- `xit`/`skip` без `TODO`-комментария.
...
4. **Пишем 3 unit**, **4 component** теста. Покрытие в Vitest `--coverage`.
...
10. **Badge** статуса в README + раздел «Тесты и CI».
...
- [ ] В тестах нет проверки классов/стилей — только поведение.
- [ ] Prettier не конфликтует с ESLint.

## Что дальше
Дальше — **модуль 13 Next.js и backend**: App Router, Server Components, Server Actions, API routes. Будешь писать fullstack, не переключаясь между проектами.
