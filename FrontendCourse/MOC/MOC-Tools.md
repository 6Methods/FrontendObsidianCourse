---
тип: MOC
тема: Инструменты
---

# MOC — Инструменты

Карта по инструментам разработчика: Git, DevTools, сборщик, линт, тесты, CI.

## Git (модуль 08)
- [[08.1-Git-основы]]
- [[08.2-Ветки-и-merge]]
- [[08.3-GitHub-и-PR]]

## DevTools (модуль 08)
- [[08.4-DevTools-Elements-и-Console]]
- [[08.5-DevTools-Network-и-Sources]]
- [[08.6-DevTools-Application-и-Performance]]

## Сборщик и dev-окружение (модуль 12)
- [[12.1-Vite-в-деталях]]

## Линт и форматирование (модуль 12)
- [[12.2-ESLint-и-Prettier]]
- [[12.3-Линтер-для-TS-React]]

## Тестирование (модуль 12)
- [[12.4-Vitest]]
- [[12.5-Testing-Library]]
- [[12.6-Playwright]]

## CI/CD (модуль 12)
- [[12.7-CI-на-GitHub-Actions]]

## Практика
- [[M08-Git-workflow-и-debug]] — PR-workflow + отладка через DevTools.
- [[M12-Тесты-и-CI]] — обкладываем SPA тестами и CI.
